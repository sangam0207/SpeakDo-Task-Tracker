...
const groq = new Groq({
  apiKey: ENV.GROQ_API_KEY, // Get free key from https://console.groq.com
});

export const parseTranscriptWithGroq = async (text) => {
  const currentDate = new Date().toISOString().split("T")[0];
...
// Get API key: https://console.groq.com (free, no credit card required)